...
$details=mysqli_fetch_array($getCid);
	...
	$sqlP=mysqli_query($con,"INSERT INTO receipts(customer_id,total,payType,serialno,served_by)
				VALUES('{$details['customer_id']}','{$amount}','{$pType}','{$serial}','{$_SESSION['username']}')  ");
...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		foreach($data as $row) {
			...
			$this->Cell($w[1], 4, number_format((float)$row[1],2), 'LR', 0, 'R', $fill);
			$this->Cell($w[2], 4, $row[2], 'LR', 0, 'L', $fill);
			$this->Cell($w[3], 4, number_format((float)$row[3],2), 'LR', 0, 'R', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
$titling= <<<EOD
<strong> <font style="font-size:11">Pharmacy Sys</font> </strong> <br> <strong>Official Receipt</strong><br> E-mail: pharmacysys@yahoo.com 
<br>-----------------------------------------
EOD;
...
$data = $pdf->LoadData('C:/xampp/htdocs/Pharmacy/receipts/docs/'.$recNo.'.txt');
...
$pdf->Output('C:/xampp/htdocs/Pharmacy/receipts/printouts/'.$details1['customer_name'].time().'-receipt.pdf','F');
//unlink('C:/xampp/htdocs/Pharmacy/receipts/docs/'.$recNo.'.txt');
